...
def set_workspace(directory: str | Path) -> None:
    """把 v0.8 的所有文件工具限制在本次评测工作目录。"""
    global WORKSPACE
...
def workspace_path(path: str) -> Path:
    ...
    try:
        ...
        raise ValueError(f"路径超出评测工作目录：{path}") from exc
    return candidate
...
TOOL_SCHEMAS.append(
    {
        ...
        "function": {
            "name": "run_tests",
            "description": "在指定目录运行 python -m pytest -q，不执行其他 shell 命令",
            "parameters": {
                "type": "object",
                "properties": {
                    "directory": {
                        "type": "string",
                        "description": "测试项目目录，默认当前目录",
                    }
                },
            },
        },
